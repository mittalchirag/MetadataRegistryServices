angular.module('mainControllersModule', [])
    ...
    .controller('authCtrl', function (Auth, $timeout, $location, $rootScope, $window, $interval, User, AuthToken, $scope) {
        var app = this;
    ...
    .controller('registerCtrl', function ($location, $timeout, User, $scope) {
        var app = this;
        app.groupsList = false;
        app.roles = ['Editor', 'Publisher', 'Author', 'Reader'];
        app.getGroups = function () {
            User.getGroups().then(function (data) {
                if (data.data.success) {
                    console.log(data.data.groups);
                    app.groupsList = data.data.groups;
                } else {
        ...
        app.regUser = function (regData) {
            ...
            let temp1 = app.regData;

            sendRegData.username = temp1.username;
            sendRegData.name = temp1.name;
            sendRegData.password = temp1.password;
            sendRegData.groupRolesId = [];

            let k = 0;
            if (temp1.group != undefined) {
                let lengthToIterate = parseInt(Object.keys(temp1.group)[Object.keys(temp1.group).length - 1]) + 1;
                for (let j = 0; j < lengthToIterate; j++) {
                    if (temp1.group[j] != undefined) {
                        if (temp1.group[j].groupRoleId != false) {
                            sendRegData.groupRolesId[k++] = {
                                groupRoleId: temp1.group[j].groupRoleId
                            }
                        }
                    }
                }
            ...
            app.errorMsg = false;
            User.create(sendRegData).then(function (data) {
                if (data.data.success) {
